...
namespace PublicComponentCenter
{
    ...
    /// Android平台
    /// </summary>
    public class PlatformHarmony : AppPlatformBase
    {
        #region Interface
        ...
        {
            CallHarmony("GameVideo",args.FirstOrDefault()?.ToString());
            return true;
        }
        ...
        private bool Call(string method, params object[] args)
        {
            ...
            {
                $"调用Android层参数为【{GetArgsStr(args)}】的方法【{method}】时发生错误：{e.Message}".Log();
        ...
        private void CallHarmony(string methodName, params object[] args)
        {
#if UNITY_OPENHARMONY
            try
            {
                var obj = new OpenHarmonyJSObject("ClassHarmonyToUnity");
            ...
            {
                Debug.Log($"{nameof(CallHarmony)} : {methodName},{{{string.Join(",",args)}}}，Length : {args.Length}");
            }
#endif
        }
    }
