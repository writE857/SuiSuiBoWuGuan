...
namespace PublicComponentCenter
{
    ...
    public abstract class GameFrameworkSerializer<T>
    {
        ...
        public void RegisterSerializeCallback(byte version, SerializeCallback callback)
        {
            if (callback == null)
            {
                Debug.LogError("Serialize callback is invalid.");
            }
        ...
        public void RegisterDeserializeCallback(byte version, DeserializeCallback callback)
        {
            if (callback == null)
            {
                Debug.LogError("Deserialize callback is invalid.");
            }
        ...
        public void RegisterTryGetValueCallback(byte version, TryGetValueCallback callback)
        {
            if (callback == null)
            {
                Debug.LogError("Try get value callback is invalid.");
            }
        ...
        public bool Serialize(Stream stream, T data)
        {
            ...
            {
                Debug.LogError("No serialize callback registered.");
            }
        ...
        public bool Serialize(Stream stream, T data, byte version)
        {
            using (BinaryWriter binaryWriter = new BinaryWriter(stream, Encoding.UTF8))
            {
                ...
                {
                    Debug.LogError(Utility.Text.Format("Serialize callback '{0}' is not exist.",
                        version.ToString()));
                }
        ...
        public T Deserialize(Stream stream)
        {
            using (BinaryReader binaryReader = new BinaryReader(stream, Encoding.UTF8))
            {
                ...
                {
                    Debug.LogError(Utility.Text.Format("Deserialize callback '{0}' is not exist.",
                        version.ToString()));
                }
